...
string enkripsi (string input, string key){
	string hasil = input; //inisialisasi string biar string hasil panjang sama, gampange disamakan dengan input
...
int main (){
	...
	transform(key.begin(), key.end(), key.begin(), ::toupper); //edom atas
	cout << "Hasil enkripsi dari plaintext : " << enkripsi(input, key) << endl;
	cout << "Hasil dekripsi dari cipher text : " << dekripsi(enkripsi(input, key), key) << endl;
}
